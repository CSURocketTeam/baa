...
void config::init(int argc, char** argv) {
	desc.add_options()
	    ...
	    ("audio-backend", po::value<std::string>(&audio_backend_)->default_value("alsa"), "Audio backend: alsa,loopback,portaudio,wave,null")
	    ...
	    ("tx-delay", po::value<int>(&tx_delay_)->default_value(666), "set tx-delay in ms")
	    ...
	    ("out-chan", po::value<int>(&out_channels_count_)->default_value(1), "output channel count")
	    ...
	    ("callsign,c", po::value<std::string>(&callsign_), "user's amateur radio callsign")
	    ("lat", po::value<std::string>(&lat_)->default_value("0000.00N"), "latitude in format ddmm.mmN")
	    ("lng", po::value<std::string>(&lng_)->default_value("00000.00W"), "longitude in format dddmm.mmW")
	    ("course", po::value<int>(&course_), "track angle in degrees")
	    ("speed", po::value<int>(&speed_), "ground speed in knots")
	    ("alt", po::value<int>(&alt_), "altitude in feet")
	;
